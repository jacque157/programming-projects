...
from PointNet2Modules import Loss
from PointNetpp import PartSegmentation
...
class Trainer:
    def __init__(self, batch_size=10, training_p=0.7, validation_p=0.2, seed=42, path='models'):
        ...
        number_of_joints = 15
        self.pointNet = PartSegmentation(3, number_of_joints).to(self.device)
        self.pointNet.apply(self.weights_init)
        self.data_path = path
        ...
        self.training_loader = data.DataLoader(self.dataset, batch_size=batch_size, sampler=train_sampler)
        self.validation_loader = data.DataLoader(self.dataset, batch_size=batch_size, sampler=validation_sampler)
        self.testing_loader = data.DataLoader(self.dataset, batch_size=batch_size, sampler=testing_sampler)

        self.optimiser = optim.Adam(self.pointNet.parameters(), lr=0.001, betas=(0.9, 0.999))
    ...
    def train(self, epochs, validate=False, start=0):
        ...
        for epoch in range(start, epochs):
            ...
            for batch_index, cloud in enumerate(self.training_loader):
                ...
                prediction, _ = self.pointNet(point_cloud)
            ...
            if validate:
                validation_loss = self.validate_training(epoch)
    ...
    def save_state(self, path, epoch):
        torch.save(self.pointNet.state_dict(), f"{path}/model_{epoch}.pth")
    ...
    def load_state(self, path, epoch):
        self.pointNet.load_state_dict(torch.load(f"{path}/model_{epoch}.pth"))
    ...
    def validate_training(self, epoch):
        ...
        for batch_index, cloud in enumerate(self.validation_loader):
            point_cloud, annotation = self.get_data_and_ground_truth(cloud)
            prediction, _ = self.pointNet(point_cloud)
            loss = self.validation_loss(prediction, annotation)
            running_loss += loss.item()
            if batch_index % 20 == 0:
                print('-' * 40)
