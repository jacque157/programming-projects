...
from point_net import PointNetPartSegmentationReduced
...
class Trainer:
    def __init__(self, batch_size=10, training_p=0.7, validation_p=0.2, seed=42, path='models'):
        ...
        number_of_joints = 15
        self.pointNet = PointNetPartSegmentationReduced(self.device, number_of_joints).to(self.device)
        self.data_path = path
        self.loss_function = PointNetLoss(self.device)
        self.validation_loss = PointNetLoss(self.device)
    ...
    def validate_training(self, epoch):
        ...
        for batch_index, cloud in enumerate(self.validation_loader):
            point_cloud, annotation = self.get_data_and_ground_truth(cloud)
            mat3x3, mat64x64, prediction = self.pointNet(point_cloud)
            loss = self.validation_loss(mat64x64, prediction, annotation)
            running_loss += loss.item()
            if batch_index % 20 == 0:
                print('-' * 40)
