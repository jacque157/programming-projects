...
class Tester:
    ...
    def test(self):
        ...
        print(f"\nParameters: {Tester.count_parameters(self.pointNet)}")
    ...
    def generate_images(self, acc=0.9):
        ...
        for batch_index, cloud in enumerate(self.testing_loader):
            ...
            for i in range(len(point_cloud_b)):
                ...
                if self.device == 'cuda':
                    Sketch.plot_annotated_ptcloud(cloud.cpu().numpy(), pose_p.detach().cpu().numpy())
                    Sketch.plot_annotated_ptcloud(cloud.cpu().numpy(), pose_t.detach().cpu().numpy())
                    Sketch.plot_difference(cloud.cpu().numpy(), pose_t.detach().cpu().numpy(), pose_p.detach().cpu().numpy())
                else:
                    Sketch.plot_annotated_ptcloud(cloud.numpy(), pose_p.detach().numpy())
                    Sketch.plot_annotated_ptcloud(cloud.numpy(), pose_t.detach().numpy())
                    Sketch.plot_difference(cloud.numpy(), pose_t.detach().numpy(), pose_p.detach().numpy())
...
if __name__ == '__main__':
    ts = Tester('models/PartSegmentationReducedTr3/model_72.pth', 8)
    ts.test()
    #ts.generate_images()
