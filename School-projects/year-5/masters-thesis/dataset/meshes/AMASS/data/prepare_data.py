...
def dump_amass2pytroch(datasets, amass_dir, out_posepath, logger = None, rnd_seed = 100, keep_rate = 0.01):
    ...
    for ds_name in datasets:
        ...
        for npz_fname in tqdm(npz_fnames):
            ...
            list_ = []
            for _ in cdata_ids:
                str_ = str(cdata['gender'])
                if str_[0] == 'b':
                    str_ = str_[2:-1]
                list_.append(gdr2num[str_])
            data_gender.extend(list_) 
                #data_gender.extend([gdr2num[str(cdata['gender'])] for _ in cdata_ids])
...
if __name__ == '__main__':
    ...
    work_dir = '../../../../data'#makepath('WHERE_YOU_WANT_YOUR_FILE_TO_BE_DUMPED/%s' % (expr_code))
    ...
    amass_splits = {
        ...
        'train': ['CMU', 'MPI_Limits', 'TotalCapture', 'Eyes_Japan_Dataset', 'KIT', 'ACCAD', 'EKUT', 'TCD_handMocap'] #ACCAD#'train': ['CMU', 'MPI_Limits', 'TotalCapture', 'Eyes_Japan_Dataset', 'KIT', 'BML', 'EKUT', 'TCD_handMocap']#ACCAD
    }
