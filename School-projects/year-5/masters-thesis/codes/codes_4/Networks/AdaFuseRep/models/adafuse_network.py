...
class MultiViewPose(nn.Module):
    ...
    def forward(self, inputs, **kwargs):
        ...
        standard_cam_T = - torch.bmm(cam_R.reshape(-1,3,3), cam_T.reshape(-1,3,1)).view(batch, nview, 3, 1)
    ...
    def collate_first_two_dims(self, tensor):
        ...
        dim1 = tensor.shape[1]
        left = tensor.shape[2:]
        return tensor.reshape(dim0 * dim1, *left)
    ...
    for b in range(batch):
        for j in range(njoint):
            ...
            for i in range(niter):
                sampled_views = random.sample(sorted(view_set), 2)
