...
from Trainers.RelativePoseRegressionTrainer import absolute_pose
...
DEVICE = 'cuda:0'
EXPERIMENT_NAME = f'GoogleNetRegressionRelativePoseEstimation'
BATCH_SIZE = 16
BEST_MODEL = 3
...
def compute_error(dataloader, network):
    ...
    for i, sample in enumerate(dataloader):
        ...
        skeletons_targets_3D = project_camera_skeletons(skeletons_targets,
                                                        sample['rotation_matrix_inverted'].double(),
                                                        sample['translation_vector_inverted'].double())
        for skeleton_predicted, skeleton_target in zip(predicted_skeleton.detach().cpu(), skeletons_targets.detach().cpu()):
