...
class ToTensor(object):
    ...
    def __call__(self, sample : dict[str, np.array]) -> dict[str, torch.Tensor]:
        ...
        if point_clouds.ndim == 3:
            point_clouds = np.moveaxis(point_clouds, 2, 1)
        else:
            point_clouds = np.moveaxis(point_clouds, 3, 1)
...
class ToPointCloud(object):
    ...
    def __call__(self, sample : dict[str, np.array]) -> dict[str, np.array]:
        ...
        unorganised = []
        for point_cloud, mask in zip(point_clouds, valid_points):
            points = point_cloud[mask]
            np.random.shuffle(points)
            points = points[: self.points] 
            if len(points) < self.points:
                padding = np.zeros((self.points - len(points), 3))
                points = np.concatenate((points, padding), 0)
            unorganised.append(points)
        sample['sequences'] = np.array(unorganised)
