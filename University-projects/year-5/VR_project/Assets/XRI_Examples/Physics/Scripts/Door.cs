...
namespace UnityEngine.XR.Content.Interaction
{
    public class Door : MonoBehaviour
    {
        ...
        //asign all lock sockets to check if keys are in the place
        public List<XRLockSocketInteractor> xrLockSocketInteractors = new List<XRLockSocketInteractor>();
        ...
        public bool CheckKeysLocked()
        {
            //if no locks
            if (xrLockSocketInteractors.Count == 0)
                return true;
            foreach (var s in xrLockSocketInteractors)
            {
                //if we find one missing, return
                if(s.hasSelection == false)
                    return false;
            }
            return true;
        }
