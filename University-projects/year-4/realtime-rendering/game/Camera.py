...
class Camera:
    def __init__(self, app, position=(0, 0, 4), yaw=-90, pitch=0, player=None):
        self.app = app
        self.player = player
        self.free = False  
        self.distance = 5
    ...
    def update(self):
        ...
        if self.player is None or self.free:
            self.move()
            self.rotate()
    ...
    def get_player_view(self):
        vertical_axis = self.player.vertical_axis 
        forward_axis = self.player.get_forward_vector()
        player_position = self.player.pos
        position = self.player.pos - (forward_axis * self.distance) + (vertical_axis * self.distance)
    ...
    def get_view(self):
        if self.player is not None and not self.free:
            return self.get_player_view()
